...
public class StoredBlock {
    ...
    public StoredBlock(Block header, BigInteger chainWork, int height) {
        this.header = header;
        COMPACT_SERIALIZED_SIZE = this.header.HEADER_SIZE_FULL + CHAIN_WORK_BYTES + 4;
    ...
    public void serializeCompact(ByteBuffer buffer) {
        ...
        buffer.put(bytes, 0, getHeader().HEADER_SIZE_FULL);  // Trim the trailing 00 byte (zero transactions).
    }
    ...
    public static StoredBlock deserializeCompact(NetworkParameters params, ByteBuffer buffer) throws ProtocolException {
        ...
        return new StoredBlock(params.getDefaultSerializer().makeBlockHeader(header), chainWork, height);
    }
