...
import org.oceanj.core.TransactionConfidence.ConfidenceType;
import org.oceanj.crypto.TransactionSignature;
import org.oceanj.script.Script;
import org.oceanj.script.ScriptBuilder;
import org.oceanj.script.ScriptError;
import org.oceanj.script.ScriptException;
import org.oceanj.script.ScriptOpCodes;
import org.oceanj.signers.TransactionSigner;
...
import static org.oceanj.core.Utils.*;
...
public class Transaction extends ChildMessage {
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, org.oceanj.script.Script, ECKey, org.oceanj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], org.oceanj.core.Transaction.SigHash, boolean)}
    ...
    /**
     * <p>Calculates a signature hash, that is, a hash of a simplified form of the transaction. How
     ...
     *
     * @param inputIndex input the signature is being calculated for. Tx signatures are always
    ...
    public synchronized Sha256Hash hashForSignatureWitness(
        ...
        ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(
            length == UNKNOWN_LENGTH ? 256 : length + 4);
        try {
            ...
            if (!anyoneCanPay) {
                ...
                for (int i = 0; i < this.inputs.size(); ++i) {
                    bosHashPrevouts
                        .write(this.inputs.get(i).getOutpoint().getHash().getReversedBytes());
                    uint32ToByteStreamLE(this.inputs.get(i).getOutpoint().getIndex(),
                        bosHashPrevouts);
                }
            ...
            if (type != SigHash.SINGLE && type != SigHash.NONE) {
                ...
                for (int i = 0; i < this.outputs.size(); ++i) {
                    ...
                    bosHashOutputs
                        .write(new VarInt(this.outputs.get(i).getScriptBytes().length).encode());
                ...
                bosHashOutputs.write(
                    new VarInt(this.outputs.get(inputIndex).getScriptBytes().length).encode());
    ...
    /**
     * Returns the confidence object for this transaction from the {@link org.oceanj.core.TxConfidenceTable}
